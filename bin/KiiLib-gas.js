...
(function (gas) {
    var GASClient = (function () {
        ...
        GASClient.prototype.setUrl = function (url) {
            this.url = url;
        };

        GASClient.prototype.setMethod = function (method) {
            this.params['method'] = method;
        };

        GASClient.prototype.setContentType = function (value) {
            this.params['contentType'] = value;
        };

        GASClient.prototype.setHeader = function (key, value) {
            this.headers[key] = value;
        };

        GASClient.prototype.setKiiHeader = function (context, authRequired) {
        ...
        GASClient.prototype.sendText = function (text, callback) {
            this.params['payload'] = text;
            this.sendRequest(callback);
        };

        GASClient.prototype.sendJson = function (json, callback) {
            this.sendText(JSON.stringify(json), callback);
        };

        GASClient.prototype.send = function (callback) {
            this.sendRequest(callback);
        };

        GASClient.prototype.sendRequest = function (callback) {
            var result = UrlFetchApp.fetch(this.url, this.params);
            var status = result.getResponseCode();
            if (status == 204) {
                callback.onReceive(status, result.getHeaders(), {});
            } else if (200 <= status && status < 300) {
                callback.onReceive(status, result.getHeaders(), JSON.parse(result.getContentText()));
            } else {
